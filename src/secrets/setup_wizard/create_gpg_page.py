...
@Gtk.Template(resource_path="/io/github/tobagin/secrets/ui/setup/create_gpg_page.ui")
class CreateGpgPage(Adw.NavigationPage):
    ...
    def _connect_signals(self):
        ...
        self.name_entry.connect("notify::text", self._on_form_changed)
        self.email_entry.connect("notify::text", self._on_form_changed)
    ...
    def _start_gpg_creation(self):
        ...
        name = self.name_entry.get_text().strip()
        email = self.email_entry.get_text().strip()
        passphrase = self.passphrase_entry.get_text().strip()
